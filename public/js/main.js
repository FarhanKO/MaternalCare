...
(function () {
  ...
  const cio = new IntersectionObserver((entries) => {
    for (const e of entries) if (e.isIntersecting) { animateCount(e.target); cio.unobserve(e.target); }
  }, { threshold: 0.4 });
  ...
  document.querySelectorAll('.progress > span[data-w]').forEach((sp) => {
    const pio = new IntersectionObserver((entries) => {
      for (const e of entries) if (e.isIntersecting) { sp.style.width = sp.dataset.w + '%'; pio.disconnect(); }
    }, { threshold: 0.3 });
    sp.style.width = '0%';
    pio.observe(sp);
  });
  ...
  if (sos) {
    ...
    sos.addEventListener('click', () => {
      status.innerHTML = '<span class="pill pill-warning">📡 Getting your location…</span>';
      const done = (locText) => {
        status.innerHTML =
